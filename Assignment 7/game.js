...
class PlayGame extends Phaser.Scene {
    ...
    create() {
        ...
        for(let i = 0; i < 20; i++) {
            let ground = this.groundGroup.create(Phaser.Math.Between(0, game.config.width), Phaser.Math.Between(0, game.config.height), "ground");
            ground.setScale(0.5)
            ground.refreshBody()
        }
        ...
        this.add.image(16, 16, "star").setScale(0.5)
    ...
    addGround() {
        ...
        let ground = this.groundGroup.create(Phaser.Math.Between(0, game.config.width), 0, "ground")
        ground.setScale(0.5)
        ground.refreshBody()
        this.groundGroup.setVelocityY(gameOptions.dudeSpeed / 6)

        if(Phaser.Math.Between(0, 1)) {
            let star = this.starsGroup.create(Phaser.Math.Between(0, game.config.width), 0, "star")
            star.setScale(0.5)
